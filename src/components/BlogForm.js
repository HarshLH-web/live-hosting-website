...
function BlogForm({ fields, className }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await fetch("https://webpanel.store/api/live-hosting-formData", {
        method: "POST",
        headers: {
  ...
  return (
    <div
      className={`w-full mx-auto lg:w-[80%] max-w-4xl bg-linear-to-b from-[#FEFEFC] to-[#F9F6E3] border border-[#DFDFDF] rounded-3xl ${className}`}
    >
      <form
        onSubmit={handleSubmit}
        className="flex flex-col lg:flex-row flex-wrap gap-2 w-full mx-auto p-4"
      >
        ...
        <button
          ...
          onMouseEnter={(e) =>
            ...
            (e.target.style.color = "#DE0402")
          }
          onMouseLeave={(e) =>
            !isSubmitting &&
            (e.target.style.backgroundColor = "#DE0402") &&
